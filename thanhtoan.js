...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadTourData = () => {
        ...
        if (isRebook && rebookBookingId) {
            ...
            } else if (foundTour && foundTour.status === 'confirmed') {
                // Xử lý trường hợp người dùng cố gắng vào thanh toán một tour đã xác nhận
                const staticDetails = getTourDetailsStatic(foundTour.tourId);
                tourData = { 
                    ...foundTour, 
                    name: staticDetails.name, 
    ...
    const displayTourData = () => {
        if (tourData.currentStatus === 'confirmed') {
            displayError(`Tour **${tourData.name}** đã được thanh toán và xác nhận. Vui lòng kiểm tra trang <a href="tour-dachon.html">Quản lý Tour</a>.`, 'green', 'check-circle');
            return;
        }
        ...
        document.getElementById('checkout-form').addEventListener('submit', function(e) {
            ...
            if (isRebook && rebookBookingId) {
                ...
                const newBooking = {
                    ...
                    name: tourData.name, // 🔥 ĐÃ BỔ SUNG TRƯỜNG NÀY (tourData.name đã được lấy ở loadTourData)
                    date: tourData.date, 
                    quantity: tourData.quantity,
                    price: tourData.price,
                    status: 'confirmed', // Coi như đã thanh toán thành công
                ...
                alert(`Đặt lại Tour và Thanh toán thành công! Mã booking mới của bạn là: ${newBookingId}`);
                updateSuccess = true;

            } else {
                // --- LOGIC THANH TOÁN LẦN ĐẦU (Pending -> Confirmed) ---
                ...
                if (bookingIndex !== -1) {
                    // Cập nhật trạng thái và thông tin khách hàng
                    allTours[bookingIndex].status = 'confirmed'; 
                    ...
                    alert('Thanh toán thành công! Tour của bạn đã được xác nhận.');
                    updateSuccess = true;
                }
            }
